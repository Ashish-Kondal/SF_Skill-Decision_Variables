...
input_dir <- paste("sample_data/", sep="")  		 # Setting working directory and suffix as file address
output_dir <- paste("sample_data/output/", sep="") 		 # Setting output directory to store files and final images
...
huc6_shapefile_address <- paste("Shapefiles/PNW_Shapefile_USonly/pnw_shapefile_USonly.shp", sep="")
wa_shapefile_address <- paste("Shapefiles/WA_Shapefile/WA_State_Boundary.shp", sep="")
or_shapefile_address <- paste("Shapefiles/OR_Shapefile/Oregon.shp", sep="")
id_shapefile_address <- paste("Shapefiles/Idaho_shapefile/Shape/GU_StateOrTerritory.shp", sep="")
...
#-------------------------------------------------------------------------------
## 			EXTRACTING ELEVATION DATASET FOR US PORTION OF PNW
#-------------------------------------------------------------------------------	
```{r Extracting Elevation Data }
elev_inputfilename <- paste("PNW_Elevation.txt",sep="") 				
elevation <- read_csv(file = elev_inputfilename,col_names = TRUE, show_col_types = FALSE)		

HSS_inputfilename <- paste(input_dir, "GridWise_Tercile_HeidkeSkillScore_Fall_Fertilization_InitMonth_09_PNW_BothThreshold_NDJF.txt",sep="")					
HSS_data <- read_csv(file = HSS_inputfilename, col_names = TRUE, show_col_types = FALSE)# Reading and only selecting particular columns to save memory 
merged_data <- merge(HSS_data,elevation,by=c("Lat","Lon"))	

elevation <- dplyr::select(merged_data,c("Lat","Lon","Elevation"))
elev_outputfilename <- paste(output_dir,"PNW_Elevation_forUSportionOnly.txt",sep="") 
write_csv(x = elevation, file = elev_outputfilename , append = FALSE, col_names = TRUE)
...
#-------------------------------------------------------------------------------
## 		PNW's CROPLAND - IRRIGATED & NON-IRRIGATED GRIDS
#-------------------------------------------------------------------------------	
```{r Cropland Grids}
vegetation_file <- paste("PNW_crop_coverage_with_area_2024.txt",sep="")
...
for (init in c(11,10,9)){
  HSS_inputfilename <- paste(input_dir, "HUC0",huc,"_Tercile_HeidkeSkillScore_Fall_Fertilization_InitMonth_0",init,"_PNW_BothThreshold_NDJF.txt",sep="")
